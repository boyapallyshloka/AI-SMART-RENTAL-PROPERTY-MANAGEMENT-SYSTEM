...
def train_and_evaluate():
    ...
    df = load_and_prepare()
    train, test = time_based_split(df)  # uses the CORRECTED split now defined in preprocessing.py
    ...
    preds_clipped = preds.clip(min=0)

    mae = mean_absolute_error(y_test, preds_clipped)
    rmse = mean_squared_error(y_test, preds_clipped) ** 0.5

    y_test_bin = (y_test > 0).astype(int)
    preds_bin = (preds_clipped > 0.5).astype(int)
    acc = accuracy_score(y_test_bin, preds_bin)
    f1 = f1_score(y_test_bin, preds_bin)

    roc_auc = roc_auc_score(y_test_bin, preds_clipped)
    ...
    if actual_nonzero_mask.sum() > 0:
        regressor_only_mae = mean_absolute_error(y_test[actual_nonzero_mask], preds_clipped[actual_nonzero_mask])
        regressor_only_rmse = mean_squared_error(y_test[actual_nonzero_mask], preds_clipped[actual_nonzero_mask]) ** 0.5
    else:
    ...
    joblib.dump(model, f"{ARTIFACTS_DIR}/model_m3_v1_rf.pkl")

    metrics = {
        "model_version": "m3_v1_rf",
    ...
    with open(f"{ARTIFACTS_DIR}/metrics_rf.json", "w") as f:
        json.dump(metrics, f, indent=2)

    print(f"\nModel and metrics saved to {ARTIFACTS_DIR}")
